...
from .db import ensure_schema, DATABASE_URL, get_connection, put_connection, execute_query, get_db_connection
...
from backend.monitoring import record_error
from backend.feature_blacklist import find_matching_features
from backend.clickable_entities import extract_clickable_entities, ClickableEntity
from backend.db_queries import (
    QUERY_GET_USER_BY_EMAIL,
    QUERY_GET_USER_BY_OAUTH,
    QUERY_GET_USER_COLLECTION,
    QUERY_GET_GAME_BY_ID,
    QUERY_SEARCH_GAMES_SINGLE_WORD,
    QUERY_SEARCH_MECHANICS,
    QUERY_SEARCH_CATEGORIES,
    QUERY_SEARCH_DESIGNERS,
    QUERY_SEARCH_ARTISTS,
    QUERY_SEARCH_PUBLISHERS,
    QUERY_GET_CHAT_THREAD,
    QUERY_GET_CHAT_MESSAGES,
    QUERY_INSERT_CHAT_THREAD,
    QUERY_INSERT_CHAT_MESSAGE,
    QUERY_INSERT_CHAT_MESSAGE_WITH_METADATA,
    QUERY_UPDATE_CHAT_THREAD,
)
...
ENGINE_CONN: Optional[psycopg2_connection] = None
...
def load_user_collection(user_id: str) -> Set[int]:
    ...
    try:
        conn = get_db_connection()
        try:
            cur = execute_query(conn, QUERY_GET_USER_COLLECTION, (int(user_id),))
...
def on_startup() -> None:
    ...
    try:
        ...
        try:
            ...
            logger.error(f"Failed to connect to PostgreSQL: {db_err}")
            raise
...
def search_by_features_only(
    ...
    try:
            ...
            for req_value in required_values:
                if feature_type == "mechanics":
                    feature_conditions.append(
                        ...
                        WHERE gm2.game_id = g.id AND LOWER(m2.name) = LOWER(%s)
                    )"""
                    )
                    ...
                    feature_conditions.append(
                        ...
                        WHERE gc2.game_id = g.id AND LOWER(c2.name) = LOWER(%s)
                    )"""
                    )
                    params.append(req_value)
                    ...
                    feature_conditions.append(
                        ...
                        WHERE gd2.game_id = g.id AND LOWER(d2.name) = LOWER(%s)
                    )"""
                    )
                    params.append(req_value)
                    ...
                    feature_conditions.append(
                        ...
                        WHERE gf2.game_id = g.id AND LOWER(f2.name) = LOWER(%s)
                    )"""
                    )
                    params.append(req_value)
        ...
        if not conditions:
            ...
            sql += " ORDER BY g.num_ratings DESC NULLS LAST, g.average_rating DESC NULLS LAST LIMIT %s"
            params.append(top_k)
        else:
        ...
        try:
            try:
                cur = execute_query(conn, sql, tuple(params) if params else None)
            except Exception as e:
            ...
            for row in cur.fetchall():
                ...
                # Note: get_game_features is imported at top of file

                try:
                    game_features = get_game_features(conn, game_id)
                except Exception as e:
                ...
                try:
                    ...
                    logger.debug(f"Error fetching designers for game {game_id}: {e}")
                    designers = []
...
def oauth_callback(req: OAuthCallbackRequest):
    ...
    try:
        ...
        cur = execute_query(conn, QUERY_GET_USER_BY_OAUTH, (req.provider, oauth_id))
        user = cur.fetchone()
...
def email_login(req: EmailLoginRequest):
    ...
    conn = get_db_connection()
    try:
        cur = execute_query(conn, QUERY_GET_USER_BY_EMAIL, (req.email,))
        user = cur.fetchone()
...
def get_current_user_info(current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    if "is_admin" not in current_user:
        ...
        try:
            query = "SELECT is_admin FROM users WHERE id = %s"
            cur = execute_query(conn, query, (current_user["id"],))
            row = cur.fetchone()
...
def update_username(req: UsernameUpdateRequest, current_user: Dict[str, Any] = Depends(get_current_user_required)):
    ...
    try:
        ...
        cur = execute_query(conn, query, (username, current_user["id"]))
        if cur.fetchone():
            raise HTTPException(status_code=400, detail="Username already taken")

        logger.info(f"Updating username for user {current_user['id']} to {username}")
        query = "UPDATE users SET username = %s WHERE id = %s"
        execute_query(conn, query, (username, current_user["id"]))
        conn.commit()
...
def update_bgg_id(req: BggIdUpdateRequest, current_user: Dict[str, Any] = Depends(get_current_user_required)):
    ...
    try:
        ...
        query = "UPDATE users SET bgg_id = %s WHERE id = %s"
        execute_query(conn, query, (bgg_id, current_user["id"]))
        conn.commit()
...
def import_bgg_collection(current_user: Dict[str, Any] = Depends(get_current_user_required)):
    ...
    conn = get_db_connection()
    try:
        # personal_rating column should exist in PostgreSQL schema
        ...
        for game_data in games_data:
            ...
            query = "SELECT personal_rating FROM user_collections WHERE user_id = %s AND game_id = %s"
            cur = execute_query(conn, query, (current_user["id"], game_id))
            ...
            if existing:
                ...
                if personal_rating is not None:
                    try:
                        query = "UPDATE user_collections SET personal_rating = %s WHERE user_id = %s AND game_id = %s"
                        execute_query(conn, query, (personal_rating, current_user["id"], game_id))
                        conn.commit()
                        updated_count += 1
                    except Exception as e:
                ...
                try:
                    query = "INSERT INTO user_collections (user_id, game_id, personal_rating) VALUES (%s, %s, %s)"
                    execute_query(conn, query, (current_user["id"], game_id, personal_rating))
                    conn.commit()
                    added_count += 1
                except Exception as e:
    ...
    try:
        ...
        for row in cur.fetchall():
            if row[0] not in seen_ids:
                game_id = row[0]
                # Skip features for @ search to make it faster (per user request)
                # features = []  # Not needed for @ search
                ...
                game_results.append(
                    {
                        ...
                        "average_rating": row[4],
                        "num_ratings": row[5],
                        "features": [],  # Removed features for performance
                    }
                )
        ...
        # Search mechanics
        try:
            cur_m = execute_query(conn, QUERY_SEARCH_MECHANICS, (search_pattern, feature_limit))
            mechanics_rows = cur_m.fetchall()
            for row in mechanics_rows:
                if len(row) >= 2:
                    feature_results.append({"type": "mechanics", "id": row[0], "name": row[1], "icon": "⚙️"})
        except Exception as e:
            logger.error(f"Error searching mechanics: {e}")

        # Search categories
        try:
            cur_c = execute_query(conn, QUERY_SEARCH_CATEGORIES, (search_pattern, feature_limit))
            categories_rows = cur_c.fetchall()
            for row in categories_rows:
                if len(row) >= 2:
                    feature_results.append({"type": "categories", "id": row[0], "name": row[1], "icon": "🏷️"})
        except Exception as e:
            logger.error(f"Error searching categories: {e}")

        # Search designers
        try:
            cur_d = execute_query(conn, QUERY_SEARCH_DESIGNERS, (search_pattern, feature_limit))
            designers_rows = cur_d.fetchall()
            for row in designers_rows:
                if len(row) >= 2:
                    feature_results.append({"type": "designers", "id": row[0], "name": row[1], "icon": "👤"})
        except Exception as e:
            logger.error(f"Error searching designers: {e}")

        # Search artists
        try:
            cur_a = execute_query(conn, QUERY_SEARCH_ARTISTS, (search_pattern, feature_limit))
            artists_rows = cur_a.fetchall()
            for row in artists_rows:
                if len(row) >= 2:
                    feature_results.append({"type": "artists", "id": row[0], "name": row[1], "icon": "🎨"})
        except Exception as e:
            logger.error(f"Error searching artists: {e}")

        # Search publishers
        try:
            cur_p = execute_query(conn, QUERY_SEARCH_PUBLISHERS, (search_pattern, feature_limit))
            publishers_rows = cur_p.fetchall()
            for row in publishers_rows:
                if len(row) >= 2:
                    feature_results.append({"type": "publishers", "id": row[0], "name": row[1], "icon": "🏢"})
        except Exception as e:
            logger.error(f"Error searching publishers: {e}")

        result = {"games": game_results, "features": feature_results}
        ...
        error_msg = str(e)
        record_error("database_connection", error_msg, {"endpoint": "/games/search", "query": q})
        logger.error(f"Database connection error in search: {e}", exc_info=True)
        # Try to get a new connection and retry once
        try:
            put_connection(conn)
            conn = get_db_connection()
            ...
            for row in cur.fetchall():
                if row[0] not in seen_ids:
                    ...
                    if len(game_results) >= limit:
                        break
            return {"games": game_results, "features": []}
        except Exception as retry_error:
            logger.error(f"Retry also failed in search: {retry_error}", exc_info=True)
            return {"games": [], "features": []}
    except Exception as e:
        error_msg = str(e)
        record_error("database_query", error_msg, {"endpoint": "/games/search", "query": q})
...
def get_collection(
    sort_by: str = "year_published", order: str = "desc", current_user: Optional[Dict[str, Any]] = Depends(get_current_user)
):
    ...
    conn = get_db_connection()
    try:
        # personal_rating column exists in PostgreSQL schema
...
def add_to_collection(req: AddToCollectionRequest, current_user: Dict[str, Any] = Depends(get_current_user_required)):
    ...
    try:
        # Verify game exists
        cur = execute_query(conn, QUERY_GET_GAME_BY_ID, (game_id,))
        if not cur.fetchone():
        ...
        execute_query(conn, insert_sql, (current_user["id"], game_id))
        conn.commit()
...
def remove_from_collection(game_id: int, current_user: Dict[str, Any] = Depends(get_current_user_required)):
    ...
    conn = get_db_connection()
    try:
        query = "DELETE FROM user_collections WHERE user_id = %s AND game_id = %s"
        cur = execute_query(conn, query, (current_user["id"], game_id))
        conn.commit()
...
@app.get("/profile/export-data")
def export_user_data(current_user: Dict[str, Any] = Depends(get_current_user_required)):
    """Export all user data as JSON."""
    user_id = current_user["id"]
    conn = get_db_connection()
    try:
        # Get user info
        query = "SELECT id, email, username, bgg_id, oauth_provider, is_admin, created_at FROM users WHERE id = %s"
        cur = execute_query(conn, query, (user_id,))
        ...
        user_data = {
            ...
            "username": user_row[2],
            "bgg_id": user_row[3],
            "oauth_provider": user_row[4],
            "is_admin": bool(user_row[5]),
            "created_at": str(user_row[6]) if user_row[6] else None,
        }

        # Get collection
        query = """SELECT uc.game_id, g.name, g.year_published, uc.added_at, uc.personal_rating
                   FROM user_collections uc
                   JOIN games g ON uc.game_id = g.id
                   WHERE uc.user_id = %s
            ...
            collection.append(
                {
                    ...
                    "personal_rating": float(row[4]) if row[4] is not None else None,
                }
            )
        ...
        user_data["chat_threads"] = threads

        # Get chat messages (for each thread)
        for thread in threads:
            query = """SELECT id, role, message, metadata, created_at
                       FROM chat_messages
                       WHERE thread_id = %s
                       ORDER BY created_at ASC"""
            cur = execute_query(conn, query, (thread["thread_id"],))
            messages = []
            for row in cur.fetchall():
                messages.append(
                    {
        ...
        query = """SELECT fr.id, fr.question_id, fr.option_id, fr.response, fr.context,
                          fr.thread_id, fr.additional_details, fr.created_at,
                          fq.question_text
                   FROM feedback_responses fr
                   LEFT JOIN feedback_questions fq ON fr.question_id = fq.id
                   WHERE fr.user_id = %s
            ...
            feedback_responses.append(
                {
                    ...
                    "context": row[4],
                    "thread_id": row[5],
                    "additional_details": row[6],
                    "created_at": str(row[7]) if row[7] else None,
                }
            )
        user_data["feedback_responses"] = feedback_responses

        # Get scoring sessions
        query = """SELECT id, game_id, mechanism_id, intermediate_scores_json,
                          final_score, created_at, updated_at
                   FROM user_scoring_sessions
                   WHERE user_id = %s
                   ORDER BY created_at DESC"""
        cur = execute_query(conn, query, (user_id,))
        scoring_sessions = []
        for row in cur.fetchall():
            scoring_sessions.append(
                {
                    "session_id": row[0],
                    "game_id": row[1],
                    "mechanism_id": row[2],
                    "intermediate_scores": json.loads(row[3]) if row[3] else None,
                    "final_score": float(row[4]) if row[4] is not None else None,
                    "created_at": str(row[5]) if row[5] else None,
                    "updated_at": str(row[6]) if row[6] else None,
                }
            )
        user_data["scoring_sessions"] = scoring_sessions
...
def delete_own_account(current_user: Dict[str, Any] = Depends(get_current_user_required)):
    """Delete the current user's account and all associated data."""
    user_id = current_user["id"]
    conn = get_db_connection()
    try:
        # Delete in order to respect foreign key constraints
        # Delete scoring sessions
        query = "DELETE FROM user_scoring_sessions WHERE user_id = %s"
        execute_query(conn, query, (user_id,))

        # Delete feedback responses
        query = "DELETE FROM feedback_responses WHERE user_id = %s"
        execute_query(conn, query, (user_id,))

        # Delete chat messages (via threads)
        query = "SELECT id FROM chat_threads WHERE user_id = %s"
        cur = execute_query(conn, query, (user_id,))
        thread_ids = [row[0] for row in cur.fetchall()]
        if thread_ids:
            placeholders = ",".join(["%s"] * len(thread_ids))
            query = f"DELETE FROM chat_messages WHERE thread_id IN ({placeholders})"
            execute_query(conn, query, tuple(thread_ids))

        # Delete chat threads
        query = "DELETE FROM chat_threads WHERE user_id = %s"
        execute_query(conn, query, (user_id,))

        # Delete collection
        query = "DELETE FROM user_collections WHERE user_id = %s"
        execute_query(conn, query, (user_id,))
...
def get_chat_history(current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        threads = []
        for row in cur.fetchall():
            threads.append({"id": row[0], "title": row[1] or f"Chat {row[0]}", "created_at": row[2], "updated_at": row[3]})
        return threads
    finally:
        put_connection(conn)
...
def get_chat_thread(thread_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        cur = execute_query(conn, QUERY_GET_CHAT_THREAD, (thread_id, current_user["id"]))
        if not cur.fetchone():
            raise HTTPException(status_code=404, detail="Thread not found")

        # Get messages
        cur = execute_query(conn, QUERY_GET_CHAT_MESSAGES, (thread_id,))
        messages = []
        for row in cur.fetchall():
            messages.append(
                {
...
def chat(req: ChatRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)) -> ChatResponse:
    ...
    if intent == "collection_recommendation":
        ...
        if not base_game_id or not current_user:
            reply_text = (
                "I need to know which game you're asking about, and you need to be logged in to check your collection."
            )
            results = []
        else:
            try:
                ...
                if base_game_id in user_collection:
                    reply_text = "This game is already in your collection!"
                    results = []
                else:
                    # Get a connection for this section
                    conn_do_i_need = get_db_connection()
                    try:
                        # Get game features
                        from backend.reasoning_utils import get_game_features, get_feature_rarity_weights

                        game_features = get_game_features(conn_do_i_need, base_game_id)
                        ...
                        for collection_game_id in user_collection:
                            try:
                                collection_features = get_game_features(conn_do_i_need, collection_game_id)
                        ...
                        if not top_similar_games:
                            ...
                            # most_similar_game_id = top_similar_games[0][0]  # Unused for now
                            # most_similar_features = top_similar_games[0][2]  # Unused for now
                            ...
                            for feature_type in ["mechanics", "categories", "designers", "families"]:
                                rarity_weights[feature_type] = get_feature_rarity_weights(conn_do_i_need, feature_type)
                            ...
                            for collection_game_id in user_collection:
                                try:
                                    collection_features = get_game_features(conn_do_i_need, collection_game_id)
                                    for feature_type in ["mechanics", "categories", "designers", "families"]:
                                        all_collection_features[feature_type].update(
                                            collection_features.get(feature_type, set())
                                        )
                                except Exception as e:
                            ...
                            for collection_game_id, similarity, overlaps, collection_features in top_similar_games:
                                try:
                                    ...
                                    cur = execute_query(
                                        conn_do_i_need,
                                        "SELECT id, name, year_published, thumbnail, average_rating, num_ratings, min_players, max_players, description FROM games WHERE id = %s",
                                        (collection_game_id,),
                                    )
                                    row = cur.fetchone()
                                    if not row:
                                    ...
                                    try:
                                        ...
                                        logger.debug(f"Error fetching designers for collection game {collection_game_id}: {e}")
                                        designers = []
                                    ...
                                    missing_mechanics = sorted(
                                        game_features.get("mechanics", set()) - collection_features.get("mechanics", set())
                                    )
                                    missing_categories = sorted(
                                        game_features.get("categories", set()) - collection_features.get("categories", set())
                                    )
                                    missing_designers = sorted(
                                        game_features.get("designers", set()) - collection_features.get("designers", set())
                                    )
                                    missing_families = sorted(
                                        game_features.get("families", set()) - collection_features.get("families", set())
                                    )

                                    # Extra: features in collection game but not in target game
                                    extra_mechanics = sorted(
                                        collection_features.get("mechanics", set()) - game_features.get("mechanics", set())
                                    )
                                    extra_categories = sorted(
                                        collection_features.get("categories", set()) - game_features.get("categories", set())
                                    )
                                    extra_designers = sorted(
                                        collection_features.get("designers", set()) - game_features.get("designers", set())
                                    )
                                    extra_families = sorted(
                                        collection_features.get("families", set()) - game_features.get("families", set())
                                    )
        ...
        clickable_entities = extract_clickable_entities(reply_text, results)
    ...
    if intent == "recommend_similar":
        ...
        if base_game_id is None:
            ...
            has_features = (include_features and len(include_features) > 0) or (
                required_feature_values and len(required_feature_values) > 0
            )
            if not has_features:
            ...
            try:
                ...
                reply_text = "I couldn't identify which game you're asking about. Please specify a game name."
                results = []
                base_game_id = None
        ...
        ab_test_configs = {}
        conn_ab_configs = get_db_connection()
        try:
            try:
                query = "SELECT config_key, config_value FROM ab_test_configs WHERE is_active = %s"
                cur = execute_query(conn_ab_configs, query, (True,))
                for row in cur.fetchall():
                    try:
                        ab_test_configs[row[0]] = json.loads(row[1])
                    except Exception as e:
                        logger.debug(f"Error parsing A/B test config JSON for key {row[0]}, using raw value: {e}")
                        ab_test_configs[row[0]] = row[1]
            except Exception as e:
                logger.debug(f"Error loading A/B test configs: {e}")
        finally:
            put_connection(conn_ab_configs)
        ...
        if required_feature_values:
            ...
            required_feature_values = {
                k: set(v) if isinstance(v, list) else {v} if v else set() for k, v in required_feature_values.items()
            }
        ...
        try:
            ...
            if not results:
                ...
                if not results:
                    logger.debug(
                        "Retrying with embedding-only (no explain, no constraints, no features, but keeping required features)"
                    )
                    try:
                        results = ENGINE.search_similar(
                            game_id=base_game_id,
    ...
    if intent == "recommend_similar" and results and base_game_id is not None:
        ...
        if ab_configs_to_test and base_game_id is not None:
            ...
            user_ab_preferences = {}
            if current_user:
                conn_ab_prefs = get_db_connection()
                try:
                    try:
                        query = "SELECT config_key, preferred_value FROM user_ab_preferences WHERE user_id = %s"
                        cur_prefs = execute_query(conn_ab_prefs, query, (int(current_user["id"]),))
                        for row in cur_prefs.fetchall():
                            user_ab_preferences[row[0]] = row[1]
                    except Exception as e:
                        logger.debug(f"Error loading user A/B preferences: {e}")
                finally:
                    put_connection(conn_ab_prefs)
            ...
            for config_key, config_data in ab_configs_to_test:
                ...
                logger.info(
                    f"A/B test for {config_key}: A uses rarity_weighting={use_rarity_a}, B uses rarity_weighting={use_rarity_b}"
                )
                ...
                # Note: This will be called within the chat endpoint's connection context
                # We need to pass a connection, but chat endpoint will handle it
                # For now, get a temporary connection for this operation
                temp_conn = get_db_connection()
                try:
                    ab_question = _get_or_create_ab_question(temp_conn, config_key, config_data)
                finally:
                    put_connection(temp_conn)
    ...
    if current_user:
        ...
        try:
            if not thread_id:
                # Create new thread
                insert_sql = QUERY_INSERT_CHAT_THREAD

                try:
                    cur = execute_query(
                        conn, insert_sql, (current_user["id"], req.message[:50])  # Use first 50 chars as title
                    )
                    conn.commit()
                    ...
                    if is_unique_violation:
                        ...
                        try:
                            ...
                            execute_query(conn, fix_seq_query)
                            conn.commit()
                            # Retry the insert
                            cur = execute_query(conn, insert_sql, (current_user["id"], req.message[:50]))
                            conn.commit()
                            thread_id = cur.fetchone()[0]
                            logger.info(
                                f"Fixed sequence and created new chat thread {thread_id} for user {current_user['id']}"
                            )
                        except Exception as retry_error:
            ...
            # Save user message
            user_msg_sql = QUERY_INSERT_CHAT_MESSAGE
            try:
                execute_query(conn, user_msg_sql, (thread_id, "user", req.message))
            except Exception as e:
                # Handle sequence sync issues for PostgreSQL
                is_unique_violation = False
                if psycopg2 and psycopg2.errors:
            ...
            metadata = {"results": results, "query_spec": query_spec}
            assistant_msg_sql = QUERY_INSERT_CHAT_MESSAGE_WITH_METADATA
            try:
                execute_query(conn, assistant_msg_sql, (thread_id, "assistant", reply_text, json.dumps(metadata)))
            except Exception as e:
                # Handle sequence sync issues for PostgreSQL
                is_unique_violation = False
                if psycopg2 and psycopg2.errors:
                ...
                if is_unique_violation:
                    ...
                    try:
                        ...
                        logger.error(
                            f"Failed to fix sequence and retry assistant message insert: {retry_error}", exc_info=True
                        )
                        raise
                else:
                    raise
            ...
            execute_query(conn, QUERY_UPDATE_CHAT_THREAD, (thread_id,))
            conn.commit()
        finally:
            put_connection(conn)
...
async def generate_from_image(req: ImageGenerateRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            ...
            logger.debug(f"Error fetching game name for game_id {game_id}: {e}")

        # Store interaction data
        interaction_context = {"game_id": game_id, "game_name": game_name, "context": context, "api_type": api_type}

        metadata = {"game_id": game_id, "game_name": game_name, "api_type": api_type}
        ...
        execute_query(conn, insert_sql, (user_id, "image_upload", json.dumps(interaction_context), json.dumps(metadata)))
        conn.commit()
...
def explain_rules(req: RulesExplainRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        game_name = None
        try:
            cur = execute_query(conn, "SELECT name FROM games WHERE id = %s", (req.game_id,))
            row = cur.fetchone()
            if row:
                game_name = row[0]
        except Exception as e:
            logger.debug(f"Error fetching game name for game_id {req.game_id}: {e}")

        # Store interaction data
        interaction_context = {"game_id": req.game_id, "game_name": game_name, "context": req.context, "user_id": user_id}

        metadata = {"game_id": req.game_id, "game_name": game_name}

        # Store in fake_door_interactions table
        insert_sql = """INSERT INTO fake_door_interactions (user_id, interaction_type, context, metadata)
                       VALUES (%s, %s, %s, %s)"""

        execute_query(conn, insert_sql, (user_id, "rules_explainer", json.dumps(interaction_context), json.dumps(metadata)))
        conn.commit()
...
def open_scoring_pad(req: ScoringPadRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        execute_query(conn, insert_sql, (user_id, "scoring_pad", json.dumps(interaction_context), json.dumps(metadata)))
        conn.commit()
...
def get_scoring_mechanism(game_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        if not row:
            return {"exists": False, "mechanism": None}
        ...
        try:
            ...
            logger.warning(f"Error parsing criteria JSON for mechanism_id {mechanism_id}: {e}")
            criteria = {}
...
def calculate_score(req: CalculateScoreRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        cur = execute_query(conn, query, (req.mechanism_id,))
        row = cur.fetchone()
        ...
        for score_def in intermediate_scores_list:
            ...
            if formula == "value":
                ...
                if len(parts) == 2:
                    try:
                        ...
                        logger.debug(f"Error parsing multiplier in formula '{formula}', using input value directly: {e}")
                        calculated_value = float(input_value)
                else:
                    calculated_value = float(input_value)
            else:
            ...
            calculated_intermediates.append(
                {"id": score_id, "label": label, "value": calculated_value, "input_value": input_value}
            )
        ...
        return {"success": True, "intermediate_scores": calculated_intermediates, "final_score": final_score}
    except HTTPException:
        raise
    except Exception as e:
...
def save_scoring_session(req: SaveScoringSessionRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        try:
            execute_query(
                conn, insert_sql, (user_id, req.game_id, req.mechanism_id, json.dumps(req.intermediate_scores), final_score)
            )
            conn.commit()

            return {"success": True, "message": "Scoring session saved successfully"}
        finally:
            put_connection(conn)
    except Exception as e:
...
def parse_rulebook_for_scoring(
    game_id: int, rulebook_text: str, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    ...
    try:
        ...
        try:
            ...
            check_query = """SELECT id FROM scoring_mechanisms WHERE game_id = %s AND status = 'pending'"""

            cur = execute_query(conn, check_query, (game_id,))
            existing = cur.fetchone()
            ...
            if existing:
                ...
                execute_query(conn, update_query, (mechanism_data["criteria_json"], existing[0]))
                ...
                cur = execute_query(conn, insert_query, (game_id, mechanism_data["criteria_json"], "pending"))
                conn.commit()
...
def get_pending_scoring_mechanisms(current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    ...
    try:
        ...
        for row in cur.fetchall():
            try:
                ...
                logger.debug(f"Error parsing criteria JSON for mechanism_id {row[0]}: {e}")
                criteria = {}
        ...
        return {"success": True, "mechanisms": mechanisms}
    except Exception as e:
...
def review_scoring_mechanism(
    req: ReviewScoringMechanismRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    ...
    try:
        ...
        if req.status == "approved":
            ...
            get_game_query = "SELECT game_id FROM scoring_mechanisms WHERE id = %s"

            cur = execute_query(conn, get_game_query, (req.mechanism_id,))
            ...
            check_query = """SELECT id FROM scoring_mechanisms
                           WHERE game_id = %s AND status = 'approved' AND id != %s"""

            cur = execute_query(conn, check_query, (game_id, req.mechanism_id))
            existing = cur.fetchone()
            ...
            if existing:
                ...
                update_old_query = """UPDATE scoring_mechanisms
                                     SET status = 'rejected', reviewed_at = CURRENT_TIMESTAMP,
                                         reviewed_by = %s, review_notes = 'Replaced by new approved mechanism'
                                     WHERE id = %s"""

                execute_query(conn, update_old_query, (admin_user_id, existing[0]))
        ...
        execute_query(conn, update_query, (req.status, admin_user_id, req.review_notes, req.mechanism_id))
        conn.commit()

        return {"success": True, "message": f"Scoring mechanism {req.status} successfully"}
    except HTTPException:
        raise
    except Exception as e:
...
def get_game_features_endpoint(game_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    ...
    try:
        ...
        for mod in mods:
            ...
            cur = execute_query(conn, query, (feature_id,))
            row = cur.fetchone()
            if row:
                feature_name = row[0]
                if action == "add":
...
def modify_game_features(
    ...
    try:
        ...
        cur = execute_query(conn, query, (game_id, feature_type, feature_id, action))
        existing = cur.fetchone()
        ...
        query = """DELETE FROM feature_mods
               WHERE game_id = %s AND feature_type = %s AND feature_id = %s AND action = %s"""
        execute_query(conn, query, (game_id, feature_type, feature_id, opposite_action))

        # Add new modification
        query = """INSERT INTO feature_mods (game_id, feature_type, feature_id, action)
               VALUES (%s, %s, %s, %s)"""
        execute_query(conn, query, (game_id, feature_type, feature_id, action))
        conn.commit()
...
def remove_feature_modification(
    game_id: int, mod_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    """Remove a feature modification."""
    conn = get_db_connection()
    try:
        cur = execute_query(conn, "DELETE FROM feature_mods WHERE id = %s AND game_id = %s", (mod_id, game_id))
        conn.commit()
...
def search_marketplace_endpoint(game_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    """
    ...
    try:
        ...
        # Search all marketplaces (including BGA)
        listings = search_marketplace(game_name, game_id)
...
def get_random_feedback_question(current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        query = """SELECT id, question_text, question_type
               ...
               ORDER BY RANDOM()
               LIMIT 1"""
        cur = execute_query(conn, query, (True,))
        row = cur.fetchone()
        ...
        return {"id": question_id, "question_text": row[1], "question_type": row[2], "options": options if options else None}
    except Exception as e:
...
def get_helpful_question(current_user: Optional[Dict[str, Any]] = Depends(get_current_user)):
    ...
    try:
        ...
        if row:
            ...
            cur = execute_query(conn, query, ("Were these results helpful?", "single_select", True))
        ...
        query = """SELECT id, option_text FROM feedback_question_options
               WHERE question_id = %s
               ORDER BY display_order, id"""
        cur = execute_query(conn, query, (question_id,))
        options = [{"id": opt[0], "text": opt[1]} for opt in cur.fetchall()]
...
def submit_feedback_response(
    req: FeedbackResponseRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_user_required)
):
    ...
    try:
        ...
        logger.debug(
            f"Feedback response: question_id={question_id}, option_id={option_id}, response={response}, context={context}, thread_id={thread_id}, user_id={current_user['id']}"
        )

        # Verify question exists if provided
        if question_id is not None:
            query = "SELECT id, question_type FROM feedback_questions WHERE id = %s AND is_active = %s"
            cur = execute_query(conn, query, (question_id, True))
            row = cur.fetchone()
            if not row:
            ...
            if question_type == "single_select":
                ...
                query = "SELECT id FROM feedback_question_options WHERE id = %s AND question_id = %s"
                cur = execute_query(conn, query, (option_id, question_id))
                if not cur.fetchone():
            ...
            # For multi_select, response should contain JSON array of option IDs
                ...
                if not response:
                    raise HTTPException(
                        status_code=400,
                        detail="response required for multi_select questions (should be JSON array of option IDs)",
                    )
                try:
                    ...
                    for opt_id in option_ids:
                        query = "SELECT id FROM feedback_question_options WHERE id = %s AND question_id = %s"
                        cur = execute_query(conn, query, (opt_id, question_id))
                        if not cur.fetchone():
                            raise HTTPException(
                                status_code=404, detail=f"Option {opt_id} not found or doesn't belong to question"
                            )
        ...
        if question_type == "multi_select" and response:
                ...
                for opt_id in option_ids:
                    ...
                    try:
                        ...
                        if psycopg2 and psycopg2.errors:
                            ...
                            is_unique_violation = (
                                "UniqueViolation" in str(type(e).__name__) or "duplicate key" in str(e).lower()
                            )
            ...
            logger.debug(
                f"Inserting feedback: user_id={int(current_user['id'])}, question_id={question_id}, option_id={option_id}, response={final_response}, context={context}, thread_id={thread_id}"
            )
            insert_sql = """INSERT INTO user_feedback_responses
                   (user_id, question_id, option_id, response, context, thread_id)
                   VALUES (%s, %s, %s, %s, %s, %s)"""
                ...
                if is_unique_violation:
                    ...
                    try:
                        ...
                        execute_query(
                            conn,
                            insert_sql,
                            (int(current_user["id"]), question_id, option_id, final_response, context, thread_id),
                        )
                        logger.info(f"Fixed sequence and inserted feedback response for user {current_user['id']}")
                    except Exception as retry_error:
                        logger.error(f"Failed to fix sequence and retry insert: {retry_error}", exc_info=True)
                    ...
                    if option_id:
                        ...
                        if opt_row:
                            ...
                            if is_ab_question and is_negative:
                                ...
                                execute_query(
                                    conn, """DELETE FROM user_ab_preferences WHERE user_id = %s""", (int(current_user["id"]),)
                                )
                                conn.commit()
                                logger.info(
                                    f"Cleared A/B test preferences for user {current_user['id']} after negative feedback"
                                )
            ...
            if question_id and option_id:
                cur_ab_check = execute_query(
                    ...
                    """SELECT fq.question_text, fqo.option_text
                       ...
                       JOIN feedback_question_options fqo ON fqo.id = %s
                       WHERE fq.id = %s""",
                ...
                if ab_row:
                    ...
                    if is_ab_question and is_positive:
                        ...
                        cur_configs = execute_query(
                            conn, """SELECT config_key, config_value FROM ab_test_configs WHERE is_active = %s""", (True,)
                        )
                        for config_row in cur_configs.fetchall():
                            ...
                            try:
                                ...
                                conn.commit()
                                logger.info(
                                    f"Stored A/B test preference for user {current_user['id']}: {config_key} = {preferred_value}"
                                )
        ...
        cur = execute_query(
            conn,
            "SELECT id, user_id, question_id, option_id, response, context, thread_id FROM user_feedback_responses WHERE id = (SELECT MAX(id) FROM user_feedback_responses WHERE user_id = %s)",
            (int(current_user["id"]),),
        )
        inserted_record = cur.fetchone()
        if inserted_record:
            logger.info(
                f"Feedback response saved: id={inserted_record[0]}, user_id={inserted_record[1]}, question_id={inserted_record[2]}, option_id={inserted_record[3]}, response={inserted_record[4]}, context={inserted_record[5]}, thread_id={inserted_record[6]}"
            )
        else:
...
def get_all_games(
    ...
    try:
        ...
        if search:
            ...
            count_sql = "SELECT COUNT(*) FROM games WHERE name LIKE %s"
            count_params = (search_term,)
            sql = "SELECT id, name, year_published, thumbnail FROM games WHERE name LIKE %s ORDER BY name LIMIT %s OFFSET %s"
            params = (search_term, per_page, offset)
        else:
            count_sql = "SELECT COUNT(*) FROM games"
            count_params = ()
            sql = "SELECT id, name, year_published, thumbnail FROM games ORDER BY name LIMIT %s OFFSET %s"
            params = (per_page, offset)

        cur = execute_query(conn, count_sql, count_params)
        total = cur.fetchone()[0]

        cur = execute_query(conn, sql, params)
        games = []
        for row in cur.fetchall():
            games.append({"id": row[0], "name": row[1], "year_published": row[2], "thumbnail": row[3]})
...
def get_all_feedback_questions(current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    ...
    try:
        ...
        for row in cur.fetchall():
            ...
            query_opts = """SELECT id, option_text, display_order
                   ...
                   WHERE question_id = %s
                   ORDER BY display_order, id"""
            cur_opts = execute_query(conn, query_opts, (question_id,))
            options = [{"id": opt[0], "text": opt[1], "display_order": opt[2]} for opt in cur_opts.fetchall()]
            questions.append(
                {
                    "id": question_id,
                    "question_text": row[1],
...
def create_feedback_question(
    req: FeedbackQuestionRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    ...
    try:
        ...
        try:
            ...
            insert_sql = """INSERT INTO feedback_questions (question_text, question_type, is_active)
                   VALUES (%s, %s, %s) RETURNING id"""
            cur = execute_query(conn, insert_sql, (req.question_text, req.question_type, req.is_active))
            question_id = cur.fetchone()[0]
...
def update_feedback_question(
    question_id: int, req: FeedbackQuestionRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    """Update a feedback question."""
    conn = get_db_connection()
    try:
        # Verify question exists
        query = "SELECT id FROM feedback_questions WHERE id = %s"
        cur = execute_query(conn, query, (question_id,))
        if not cur.fetchone():
            raise HTTPException(status_code=404, detail="Question not found")
        ...
        query = """UPDATE feedback_questions
               ...
               WHERE id = %s"""
        execute_query(conn, query, (req.question_text, req.question_type, req.is_active, question_id))

        # Update options
        # Delete existing options
        query = "DELETE FROM feedback_question_options WHERE question_id = %s"
        execute_query(conn, query, (question_id,))
        ...
        if req.options and req.question_type in ["single_select", "multi_select"]:
            for idx, option_text in enumerate(req.options):
                if option_text.strip():  # Only insert non-empty options
                    query = """INSERT INTO feedback_question_options (question_id, option_text, display_order)
                           VALUES (%s, %s, %s)"""
                    execute_query(conn, query, (question_id, option_text, idx))
...
def _get_or_create_ab_question(conn, config_key: str, config_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    cur = execute_query(conn, "SELECT id FROM feedback_questions WHERE question_text = %s LIMIT 1", (question_text,))
    row = cur.fetchone()
    ...
    cur = execute_query(
        conn,
        """SELECT id, option_text FROM feedback_question_options
           WHERE question_id = %s ORDER BY display_order""",
        (question_id,),
    )
    options = [{"id": opt[0], "text": opt[1]} for opt in cur.fetchall()]

    return {"id": question_id, "question_text": question_text, "question_type": "single_select", "options": options}


@app.delete("/admin/feedback/questions/{question_id}")
def delete_feedback_question(question_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    """Delete a feedback question (and its options via CASCADE)."""
    conn = get_db_connection()
    try:
        cur = execute_query(conn, "SELECT id FROM feedback_questions WHERE id = %s", (question_id,))
        if not cur.fetchone():
            raise HTTPException(status_code=404, detail="Question not found")

        execute_query(conn, "DELETE FROM feedback_questions WHERE id = %s", (question_id,))
        conn.commit()
...
def get_all_users(
    ...
    try:
        ...
        if search:
            query = """SELECT id, email, username, bgg_id, oauth_provider, is_admin, created_at
                       FROM users
                       WHERE LOWER(email) LIKE LOWER(%s) OR LOWER(username) LIKE LOWER(%s)
                       ORDER BY created_at DESC
                       LIMIT %s OFFSET %s"""
            search_term = f"%{search}%"
            cur = execute_query(conn, query, (search_term, search_term, per_page, offset))
        else:
            query = """SELECT id, email, username, bgg_id, oauth_provider, is_admin, created_at
                       FROM users
                       ORDER BY created_at DESC
                       LIMIT %s OFFSET %s"""
            cur = execute_query(conn, query, (per_page, offset))
        ...
        if search:
            count_query = """SELECT COUNT(*) FROM users
                            WHERE LOWER(email) LIKE LOWER(%s) OR LOWER(username) LIKE LOWER(%s)"""
            cur = execute_query(conn, count_query, (search_term, search_term))
        else:
...
def delete_user(user_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    ...
    try:
        ...
        if user_id == current_user["id"]:
            raise HTTPException(
                status_code=400, detail="Cannot delete your own account from admin panel. Use profile deletion instead."
            )
...
def get_ab_test_configs(current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    ...
    try:
        ...
        for row in cur.fetchall():
            try:
                ...
                logger.debug(f"Error parsing A/B test config JSON, using raw value: {e}")
                config_value = row[2]
            configs.append(
                {
                    "id": row[0],
...
def create_ab_test_config(
    ...
    try:
        ...
        cur = execute_query(conn, "SELECT id FROM ab_test_configs WHERE config_key = %s", (config_key,))
        existing = cur.fetchone()
...
def update_ab_test_config(
    ...
    try:
        ...
        execute_query(conn, f"UPDATE ab_test_configs SET {', '.join(updates)} WHERE config_key = %s", tuple(params))
        conn.commit()
...
def delete_ab_test_config(config_key: str, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    ...
    try:
        ...
        execute_query(conn, "DELETE FROM ab_test_configs WHERE config_key = %s", (config_key,))
        conn.commit()
...
# Feature Blacklist Admin Endpoints
class FeatureBlacklistRequest(BaseModel):
    keyword_phrase: str
    feature_type: Optional[str] = None  # None means all types
    match_type: str = "partial"  # "partial" or "exact"


@app.get("/admin/feature-blacklist")
def get_feature_blacklist(current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    """Get all feature blacklist rules."""
    conn = get_db_connection()
    try:
        query = """SELECT id, keyword_phrase, feature_type, match_type, is_active, created_by, created_at, updated_at
        ...
        logger.error(f"Error getting feature blacklist: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to get feature blacklist: {str(e)}")
    finally:
        put_connection(conn)


@app.post("/admin/feature-blacklist/search")
def search_features_for_blacklist(
    req: FeatureBlacklistRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    """Search for features matching a keyword/phrase to add to blacklist."""
    conn = get_db_connection()
    try:
        matches = find_matching_features(conn, req.keyword_phrase, req.feature_type, req.match_type)
        return {
            "keyword_phrase": req.keyword_phrase,
            "feature_type": req.feature_type,
            "match_type": req.match_type,
            "matches": matches,
            "total_matches": sum(len(features) for features in matches.values()),
        }
    except Exception as e:
        logger.error(f"Error searching features for blacklist: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to search features: {str(e)}")
    finally:
        put_connection(conn)


@app.post("/admin/feature-blacklist")
def add_feature_blacklist(
    req: FeatureBlacklistRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    """Add a feature blacklist rule."""
    conn = get_db_connection()
    try:
        admin_user_id = current_user["id"]

        # First, find matching features to show admin
        matches = find_matching_features(conn, req.keyword_phrase, req.feature_type, req.match_type)

        # Insert blacklist rule
        query = """INSERT INTO feature_blacklist (keyword_phrase, feature_type, match_type, created_by)
                   VALUES (%s, %s, %s, %s) RETURNING id"""
        cur = execute_query(conn, query, (req.keyword_phrase, req.feature_type, req.match_type, admin_user_id))
        ...
        return {
            "success": True,
            "rule_id": rule_id,
            "message": f"Blacklist rule added. Found {sum(len(f) for f in matches.values())} matching features.",
            "matches": matches,
        }
    except Exception as e:
        logger.error(f"Error adding feature blacklist: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to add blacklist rule: {str(e)}")
    finally:
        put_connection(conn)


@app.put("/admin/feature-blacklist/{rule_id}")
def update_feature_blacklist(
    rule_id: int, req: FeatureBlacklistRequest, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)
):
    """Update a feature blacklist rule."""
    conn = get_db_connection()
    try:
        query = """UPDATE feature_blacklist
                   SET keyword_phrase = %s, feature_type = %s, match_type = %s, updated_at = CURRENT_TIMESTAMP
                   WHERE id = %s"""
        execute_query(conn, query, (req.keyword_phrase, req.feature_type, req.match_type, rule_id))
        conn.commit()

        return {"success": True, "message": "Blacklist rule updated"}
    except Exception as e:
        logger.error(f"Error updating feature blacklist: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to update blacklist rule: {str(e)}")
    finally:
        put_connection(conn)


@app.delete("/admin/feature-blacklist/{rule_id}")
def delete_feature_blacklist(rule_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    """Delete a feature blacklist rule."""
    conn = get_db_connection()
    try:
        query = "DELETE FROM feature_blacklist WHERE id = %s"
        execute_query(conn, query, (rule_id,))
        conn.commit()
        return {"success": True, "message": "Blacklist rule deleted"}
    except Exception as e:
        logger.error(f"Error deleting feature blacklist: {e}", exc_info=True)
        raise HTTPException(status_code=500, detail=f"Failed to delete blacklist rule: {str(e)}")
    finally:
        put_connection(conn)


@app.post("/admin/feature-blacklist/{rule_id}/toggle")
def toggle_feature_blacklist(rule_id: int, current_user: Optional[Dict[str, Any]] = Depends(get_current_admin_user)):
    """Toggle a feature blacklist rule active/inactive."""
    try:
        query = """UPDATE feature_blacklist
        ...
        conn = get_db_connection()
        try:
            cur = execute_query(conn, query, (rule_id,))
            row = cur.fetchone()
            if not row:
                raise HTTPException(status_code=404, detail="Blacklist rule not found")
            conn.commit()
            return {"success": True, "is_active": row[0], "message": f"Rule {'activated' if row[0] else 'deactivated'}"}
        finally:
            put_connection(conn)
    except HTTPException:
        raise
...
def health_check():
    ...
    try:
        ...
        conn = get_db_connection()
        execute_query(conn, "SELECT 1").fetchone()

        return {"status": "healthy", "database": "connected", "engine": "loaded" if ENGINE else "not_loaded"}
    except Exception as e:
        logger.error(f"Health check failed: {e}")
        return {"status": "unhealthy", "error": str(e), "database": "disconnected" if conn is None else "error"}, 503
    finally:
        if conn:
            put_connection(conn)
